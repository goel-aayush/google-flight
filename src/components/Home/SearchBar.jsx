...
import { getSearchAirports, getSearchFlights } from "../../services/api.js";
// import SelectDateComp from "./DateComp/SelectDateComp.jsx";
...
const SearchBar = ({ bg }) => {
  ...
  const doSearch = useCallback(async (value, where) => {
    if (typeof value === "string") {
      const normalizedValue = value
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
  ...
  // debounced wrapper (stable across renders)
  const handleWhereChange = useMemo(
    () =>
      debounce((e, where) => {
        const value = e.target.value;
        // immediate fallback for empty string so UI clears quickly
  ...
  return (
    ...
    >
      ...
      <Grid
        ...
        my={1}
      >
        <Grid item="true" size={{ xs: 4, sm: "auto" }}>
          <Button
